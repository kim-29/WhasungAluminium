...
window.addEventListener('load',async ()=>{
  ...
  for (let i = 0; i < 20 && i < data.results.length; i++) {
    ...
    li.innerHTML = `
      ...
      <span>${result.properties.weight.number?result.properties.weight.number+'kg':''}</span> <span>${result.properties.price.formula.number?result.properties.price.formula.number.toLocaleString('en-US')+'원':''}</span>`;
